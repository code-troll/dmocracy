...
contract Dmocracy is Ownable {
    ...
    // New Proposal event
    event NewProposal(address indexed creator, bytes32 indexed name, string hash, uint256 indexed timestamp);

    // Vote event
    event Vote(bytes32 proposalName, uint256 votes, address voter);

    // Checks that the proposal data is valid
    modifier proposalIsValid(bytes32 name, string hash) {
        // name must not be empty
        require(name[0] != 0);

        // hash must not be empty
        bytes memory hashInBytes = bytes(hash);
        require(hashInBytes.length > 0);
    ...
    // Adds a proposal
    function addProposal(bytes32 name, string hash) proposalIsValid(name, hash) proposalNotExists(name) public returns (bool success) {
    ...
    // Get a proposal data
    function getProposal(bytes32 name) public constant returns (bool, string, uint256, uint256) {
        return (proposals[name].initialized, proposals[name].hash, proposals[name].votes, proposals[name].timestamp);
    }

    // Get a proposal hash
    function getProposalHash(bytes32 name) public constant returns (string) {
