...
contract TestNewProposal {
    ...
    function testNewProposalSuccess() public {
        ...
        bytes32 name = 0x50726f706f73616c546573743100000000000000000000000000000000000000; //ProposalTest1
        bytes32 hash = 0x3533356432313963613663643933633136326631643033363235393463326562; //535d219ca6cd93c162f1d0362594c2eb032305109e8746df86d2eaf872011d1a
        bool expected = true;
    ...
    function testNewProposalFailSameName() public {
        ...
        bytes32 name = 0x50726f706f73616c546573743200000000000000000000000000000000000000; //ProposalTest2
        bytes32 hash = 0x3533356432313963613663643933633136326631643033363235393463326562; //535d219ca6cd93c162f1d0362594c2eb032305109e8746df86d2eaf872011d1a

        Assert.equal(dmocracy.addProposal(tx.origin, name, hash), true, "The proposal could not be added!");
        Assert.notEqual(dmocracy.addProposal(tx.origin, name, hash), true, "The proposal was added but shouldn't!");
    }
